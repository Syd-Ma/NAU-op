...
export class ConsoleApp {
    ...
    async planPersonalVisit() {
        ...
        console.log("\nAvailable sessions:");
        this.printSessions(sessions);
        const selectedIds = await this.askSelectionList("Enter session numbers separated by comma", sessions.length);
    ...
    async requestCustomTour() {
        ...
        this.printExhibitions(exhibitions);
        const selectedIds = await this.askSelectionList("Enter exhibition numbers separated by comma", exhibitions.length);
    ...
    async showCustomRequests() {
        ...
        requests.forEach((request, index) => {
            console.log(`${index + 1}. ${request.visitorName} | date: ${request.requestedDate} | exhibitions: ${request.exhibitionTitles.join(", ")} | participants: ${request.participantsCount} | estimated price: ${request.estimatedPrice} | status: ${request.status}`);
        });
        console.log("");
    }
    printExhibitions(exhibitions) {
        exhibitions.forEach((exhibition, index) => {
            console.log(`${index + 1}. ${exhibition.title} | theme: ${exhibition.theme} | audience: ${exhibition.targetAudience} | hall: ${exhibition.hall} | ticket: ${exhibition.baseTicketPrice} | guided tour: ${exhibition.supportsGuidedTour ? "yes" : "no"}`);
        });
    }
    printSessions(sessions) {
        sessions.forEach((session, index) => {
            console.log(`${index + 1}. ${session.exhibitionTitle} | ${session.exhibitionDate} | ${session.opensAt}-${session.closesAt}`);
        });
    }
    printTours(tours) {
        tours.forEach((tour, index) => {
            console.log(`${index + 1}. ${tour.title} | ${tour.exhibitionTitle} | ${tour.exhibitionDate} ${tour.startsAt} | duration: ${tour.durationMinutes} min | free places: ${tour.freePlaces} | price: ${tour.pricePerVisitor}`);
        });
    }
    async ask(prompt) {
        return (await this.readline.question(`${prompt}: `)).trim();
    }
    async askNonEmpty(prompt) {
        while (true) {
    ...
    async askSelectionList(prompt, length) {
        while (true) {
            ...
            const areValid = parsed.length > 0 &&
