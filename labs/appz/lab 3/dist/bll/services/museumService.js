import { MuseumMapper } from "../mappers/museumMapper.js";
export class MuseumService {
    ...
    async findExhibitions(theme, targetAudience) {
        const exhibitions = await this.unitOfWork.exhibitions.getAll();
        const filtered = exhibitions.filter((item) => {
            ...
            const matchesAudience = targetAudience === undefined ||
    ...
    async getSessionsByDate(exhibitionDate) {
        const sessions = await this.unitOfWork.sessions.getAll();
        const exhibitions = await this.unitOfWork.exhibitions.getAll();
        return sessions
            .filter((item) => item.exhibitionDate === exhibitionDate)
            .map((item) => {
            const exhibition = this.requireEntity(exhibitions.find((exhibitionItem) => exhibitionItem.id === item.exhibitionId), `Exhibition ${item.exhibitionId} was not found for session ${item.id}.`);
            return MuseumMapper.toSessionModel(item, exhibition);
        });
    }
    async planPersonalVisit(request) {
        if (request.sessionIds.length === 0) {
            throw new Error("At least one exhibition session must be selected.");
        }
        const sessions = await this.unitOfWork.sessions.getAll();
        const exhibitions = await this.unitOfWork.exhibitions.getAll();
        const selectedSessions = request.sessionIds.map((sessionId) => this.requireEntity(sessions.find((item) => item.id === sessionId), `Session ${sessionId} was not found.`));
        ...
        const sortedSessions = [...selectedSessions].sort((left, right) => left.opensAt.localeCompare(right.opensAt));
        ...
        for (const session of sortedSessions) {
            const exhibition = this.requireEntity(exhibitions.find((item) => item.id === session.exhibitionId), `Exhibition ${session.exhibitionId} was not found.`);
            totalCost += exhibition.baseTicketPrice;
            routeParts.push(`${exhibition.title} (${session.opensAt}-${session.closesAt}, ${exhibition.hall})`);
        }
        const entity = MuseumMapper.toPersonalVisitPlanEntity(request.visitorName, plannedDate, routeParts.join(" -> "), totalCost, sortedSessions.map((item) => item.id));
        await this.unitOfWork.personalVisitPlans.add(entity);
        await this.unitOfWork.saveChanges();
        return MuseumMapper.toPersonalVisitPlanModel(entity);
    }
    async getPlannedToursByDate(exhibitionDate) {
        ...
        return tours
            ...
            .map((tour) => {
            const session = this.requireEntity(sessions.find((item) => item.id === tour.sessionId), `Session ${tour.sessionId} was not found.`);
            const exhibition = this.requireEntity(exhibitions.find((item) => item.id === session.exhibitionId), `Exhibition ${session.exhibitionId} was not found.`);
            return MuseumMapper.toPlannedTourModel(tour, session, exhibition);
        });
    }
    async bookPlannedTour(request) {
        if (request.visitorsCount < 1) {
            throw new Error("At least one visitor must be booked.");
        }
        const tour = this.requireEntity(await this.unitOfWork.plannedTours.getById(request.plannedTourId), `Planned tour ${request.plannedTourId} was not found.`);
        ...
        await this.unitOfWork.saveChanges();
        const session = this.requireEntity(await this.unitOfWork.sessions.getById(updatedTour.sessionId), `Session ${updatedTour.sessionId} was not found.`);
        const exhibition = this.requireEntity(await this.unitOfWork.exhibitions.getById(session.exhibitionId), `Exhibition ${session.exhibitionId} was not found.`);
        return MuseumMapper.toPlannedTourModel(updatedTour, session, exhibition);
    }
    async requestCustomTour(request) {
        ...
        const exhibitions = await this.unitOfWork.exhibitions.getAll();
        const selectedExhibitions = request.exhibitionIds.map((exhibitionId) => this.requireEntity(exhibitions.find((item) => item.id === exhibitionId), `Exhibition ${exhibitionId} was not found.`));
        ...
        const estimatedPrice = selectedExhibitions.reduce((sum, exhibition) => sum + exhibition.baseTicketPrice, 0) *
            request.participantsCount +
            500;
        const entity = MuseumMapper.toCustomTourRequestEntity(request.visitorName, request.requestedDate, request.exhibitionIds, request.participantsCount, estimatedPrice);
    ...
    async getCustomTourRequests() {
        const requests = await this.unitOfWork.customTourRequests.getAll();
        const exhibitions = await this.unitOfWork.exhibitions.getAll();
        return requests.map((request) => {
            const relatedExhibitions = request.exhibitionIds.map((exhibitionId) => this.requireEntity(exhibitions.find((item) => item.id === exhibitionId), `Exhibition ${exhibitionId} was not found.`));
            return MuseumMapper.toCustomTourRequestModel(request, relatedExhibitions);
        });
    }
    requireEntity(entity, errorMessage) {
        if (entity === undefined) {
