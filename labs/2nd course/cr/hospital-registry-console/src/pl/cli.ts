...
import { FileDb } from "../dal/fileDb.js";
import {
    DoctorFileRepo,
    PatientFileRepo,
    ScheduleFileRepo,
...
import { registerRepos, container } from "../bll/container.js";
import {
    DoctorService,
    PatientService,
    ScheduleService,
    AppointmentService,
} from "../bll/services.js";
import {
    DomainError,
    ValidationError,
    NotFoundError,
...
async function pause() {
    await inquirer.prompt([
        { name: "cont", type: "input", message: "Натисніть Enter, щоб продовжити..." },
    ]);
}
const opt = (v?: string) => {
    const s = (v ?? "").trim();
...
function handleError(e: unknown) {
    ...
    if (err instanceof ValidationError) console.error("Помилка валідації:", err.message);
    else if (err instanceof NotFoundError) console.error("Не знайдено:", err.message);
    else if (err instanceof DuplicateError) console.error("Дублювання:", err.message);
    else if (err instanceof SlotUnavailableError) console.error("Слот недоступний:", err.message);
    else if (err instanceof OverbookingError) console.error("Перевищено місткість:", err.message);
    else if (err instanceof DomainError) console.error("Бізнес-помилка:", err.message);
    else console.error("Несподівана помилка:", err.message);
}

const DATA_DIR = process.env.HR_DATA_DIR || "./data";
const db = new FileDb(DATA_DIR);
registerRepos({
    doctorRepo: new DoctorFileRepo(db),
    patientRepo: new PatientFileRepo(db),
    scheduleRepo: new ScheduleFileRepo(db),
    appointmentRepo: new AppointmentFileRepo(db),
});
const doctorSvc = container.resolve(DoctorService);
const patientSvc = container.resolve(PatientService);
const scheduleSvc = container.resolve(ScheduleService);
const apptSvc = container.resolve(AppointmentService);
        ...
        try {
            if (act === "add") {
                const ans = await inquirer.prompt([
                    { name: "firstName", message: "Імʼя:", validate: (v: string) => !!v || "Обовʼязково" },
                    { name: "lastName", message: "Прізвище:", validate: (v: string) => !!v || "Обовʼязково" },
                    {
                        name: "specialization",
                        message: "Спеціалізація:",
                        validate: (v: string) => !!v || "Обовʼязково",
                    },
                    { name: "phone", message: "Телефон (необовʼязково):" },
                    { name: "email", message: "Email (необовʼязково):" },
                ]);
                const d = await doctorSvc.add({
                    firstName: ans.firstName,
                    lastName: ans.lastName,
                    specialization: ans.specialization,
                    phone: opt(ans.phone),
                    email: opt(ans.email),
                });
                console.table([d]);
            ...
            if (act === "edit") {
                ...
                const { id } = await inquirer.prompt([
                    {
                        ...
                        choices: list.map((d) => ({
                            name: `${d.lastName} ${d.firstName} — ${d.specialization}`,
                            value: d.id,
                        })),
                    },
                ]);
                const current = list.find((d) => d.id === id)!;
                const upd = await inquirer.prompt([
                ...
                const saved = await doctorSvc.update({
                    ...current,
                    ...upd,
                    phone: opt(upd.phone),
                    email: opt(upd.email),
            ...
            if (act === "search") {
                const { q } = await inquirer.prompt([{ name: "q", message: "Запит:" }]);
    ...
    while (true) {
        ...
        try {
            if (act === "add") {
                ...
                const p = await patientSvc.add({
                    firstName: ans.firstName,
                    lastName: ans.lastName,
                    phone: opt(ans.phone),
                    email: opt(ans.email),
                    birthDate: opt(ans.birthDate),
                });
                console.table([p]);
            }
            ...
            if (act === "edit") {
                ...
                const saved = await patientSvc.update({
                    ...current,
                    ...upd,
                    phone: opt(upd.phone),
                    email: opt(upd.email),
                    birthDate: opt(upd.birthDate),
                });
                console.table([saved]);
            }
                ...
                const { id } = await inquirer.prompt([
                    {
                        name: "id",
                        type: "list",
                        message: "Кому додати нотатку?",
                        choices: list.map((p) => ({ name: `${p.lastName} ${p.firstName}`, value: p.id })),
                    },
                ]);
                const { note } = await inquirer.prompt([
                    { name: "note", message: "Текст нотатки:", validate: (v: string) => !!v || "Обовʼязково" },
                ]);
    ...
    while (true) {
        ...
        try {
            if (act === "add") {
                ...
                const { doctorId } = await inquirer.prompt([
                    {
                        name: "doctorId",
                        type: "list",
                        message: "Лікар:",
                ...
                const ans = await inquirer.prompt([
                    { name: "start", message: "Початок (ISO):", validate: (v: string) => !!v || "Обовʼязково" },
                    { name: "end", message: "Кінець (ISO):", validate: (v: string) => !!v || "Обовʼязково" },
                    { name: "capacity", message: "Місткість (1..):", default: "1" },
                ]);
                const slot = await scheduleSvc.addSlot({
                    doctorId,
                    start: ans.start,
                    end: ans.end,
                    capacity: parseInt(ans.capacity, 10) || 1,
                });
                console.table([slot]);
            }
            ...
            if (act === "edit") {
                ...
                const { slotId } = await inquirer.prompt([
                    {
                        name: "slotId",
                        type: "list",
                        message: "Слот:",
                        choices: slots.map((s) => ({
                            name: `${s.start} → ${s.end} [cap=${s.capacity}]`,
                            value: s.id,
                        })),
                    },
                ]);
                ...
                const saved = await scheduleSvc.updateSlot({
                    ...current,
                    start: upd.start,
                    end: upd.end,
                    capacity: parseInt(upd.capacity, 10) || 1,
                });
                console.table([saved]);
            }
    ...
    while (true) {
        ...
        try {
            ...
            if (act === "cancel") {
                ...
                const { apptId } = await inquirer.prompt([
                    {
                        name: "apptId",
                        type: "list",
                        message: "Який запис скасувати?",
                        choices: appts.map((a) => ({
                            name: `${a.id} | slot=${a.slotId} | patient=${a.patientId} | ${a.createdAt}`,
                            value: a.id,
                        })),
                    },
                ]);
...
async function menuSearch() {
    while (true) {
        ...
        try {
            if (act === "patient") {
                const { q } = await inquirer.prompt([{ name: "q", message: "ПІБ (частина):" }]);
                console.table(await patientSvc.search(q));
            }
            if (act === "doctor") {
                const { q } = await inquirer.prompt([{ name: "q", message: "ПІБ/спеціалізація (частина):" }]);
                console.table(await doctorSvc.search(q));
            }
        } catch (e) {
            handleError(e);
...
async function mainMenu() {
    while (true) {
        ...
        if (section === "doctors") await menuDoctors();
        if (section === "patients") await menuPatients();
        if (section === "schedule") await menuSchedule();
        if (section === "appt") await menuAppointments();
        if (section === "search") await menuSearch();
    }
}
